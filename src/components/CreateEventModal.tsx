...
export default function CreateEventModal({ isOpen, onClose, onCreated }: CreateEventModalProps) {
  ...
  const handleLocationInput = async (value: string) => {
    ...
    setLocation(value); // allow free text too

    if (value.length > 1 && value.length < 7 && /^\d+$/.test(value)) {
      setLoadingSuggestions(true);
      try {
        const res = await locationAPI.findByPostcode(value); // your API call
  ...
  const handleSelectSuggestion = (suggestion: any) => {
    setLocation(`${suggestion.postcode}, ${suggestion.city}`); // or whatever field you want to store
    setLocationInput(`${suggestion.postcode}, ${suggestion.city}`);
    setShowSuggestions(false);
  };
        ...
        <div className="px-6 py-5 space-y-4 max-h-[70vh] overflow-y-auto">
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              {loadingSuggestions && (
                <svg className="w-4 h-4 text-gray-400 absolute right-3.5 top-1/2 -translate-y-1/2 animate-spin" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              ...
              {showSuggestions && locationSuggestions.length > 0 && (
                <div className="absolute z-[9999] bottom-full left-0 right-0 mb-1 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl shadow-lg overflow-hidden max-h-48 overflow-y-auto">
                  {locationSuggestions.map((s, idx) => (
                    <button
                      key={idx}
                      type="button"
                      onMouseDown={() => handleSelectSuggestion(s)}
                      className="w-full text-left px-4 py-2.5 text-sm text-gray-700 dark:text-gray-300 hover:bg-indigo-50 dark:hover:bg-indigo-500/10 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors flex items-center gap-2 z-10"
                    >
                      ...
                      {s.city && <span className="text-gray-400 dark:text-gray-500 text-xs">— {s.city}</span>}
                    </button>
                  ))}
                </div>
              )}
